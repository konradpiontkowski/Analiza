...
def rysuj_boisko(
    ...
    kolor_tla: sv.Color = sv.Color(34, 139, 34),
    kolor_linii: sv.Color = sv.Color.WHITE,
    margines: int = 50,
    grubosc_linii: int = 4,
    promien_punktu: int = 8,
    skalowanie: float = 0.1
) -> np.ndarray:
    szerokosc_skalowana = int(konfiguracja.width * skalowanie)
    dlugosc_skalowana = int(konfiguracja.length * skalowanie)
    promien_kola_centrowego_skalowany = int(konfiguracja.centre_circle_radius * skalowanie)
    dystans_punktu_karnego_skalowany = int(konfiguracja.penalty_spot_distance * skalowanie)
    ...
    for poczatek, koniec in konfiguracja.edges:
        punkt1 = (int(konfiguracja.vertices[poczatek - 1][0] * skalowanie) + margines,
                  int(konfiguracja.vertices[poczatek - 1][1] * skalowanie) + margines)
        punkt2 = (int(konfiguracja.vertices[koniec - 1][0] * skalowanie) + margines,
                  int(konfiguracja.vertices[koniec - 1][1] * skalowanie) + margines)
        cv2.line(
            img=obraz_boiska,
            pt1=punkt1,
            pt2=punkt2,
...
def rysuj_diagram_voronoi_na_boisku(
    konfiguracja: KonfiguracjaBoiska,
    ...
    nakladka = cv2.addWeighted(voronoi, przezroczystosc, boisko, 1 - przezroczystosc, 0)
