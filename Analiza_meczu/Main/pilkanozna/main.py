...
from PilkaPliki.funkcje.pilka import SledzicielPilki, OznacznikPilki
...
KOLORY = ['#FF1493', '#00BFFF', '#FF6347', '#FFD700']
OZNACZNIK_ETYKIEK_WIERZCHOŁKÓW = sv.VertexLabelAnnotator(
...
OZNACZNIK_ETYKIEK_RAMKI = sv.LabelAnnotator(
    color=sv.ColorPalette.from_hex(KOLORY),
    text_color=sv.Color.from_hex('#FFFFFF'),
    text_padding=5,
    text_thickness=1,
)
OZNACZNIK_ETYKIEK_ELIPSY = sv.LabelAnnotator(
    color=sv.ColorPalette.from_hex(KOLORY),
    text_color=sv.Color.from_hex('#FFFFFF'),
    text_padding=5,
    text_thickness=1,
...
def uzyskaj_przycinki(ramka: np.ndarray, detekcje: sv.Detections) -> List[np.ndarray]:
...
def uruchom_detekcje_boiska(sciezka_wideo: str, urzadzenie: str) -> Iterator[np.ndarray]:
    ...
    for ramka in generator_ramki:
        ...
        ramka_oznaczona = OZNACZNIK_ETYKIEK_WIERZCHOŁKÓW.annotate(
...
def uruchom_detekcje_gracza(sciezka_wideo: str, urzadzenie: str) -> Iterator[np.ndarray]:
    ...
    for ramka in generator_ramki:
        ...
        ramka_oznaczona = OZNACZNIK_ETYKIEK_RAMKI.annotate(ramka_oznaczona, detekcje)
        yield ramka_oznaczona
...
def uruchom_detekcje_pilki(sciezka_wideo: str, urzadzenie: str) -> Iterator[np.ndarray]:
    ...
    generator_ramki = sv.get_video_frames_generator(source_path=sciezka_wideo)
    sledziciel_pilki = SledzicielPilki(buffer_size=20)
    ...
    for ramka in generator_ramki:
        detekcje = tnący(ramka).with_nms(threshold=0.1)
        detekcje = sledziciel_pilki.update(detekcje)
        ramka_oznaczona = ramka.copy()
...
def uruchom_sledzenie_graczy(sciezka_wideo: str, urzadzenie: str) -> Iterator[np.ndarray]:
    model_detekcji_gracza = YOLO(SCIEZKA_MODELU_DETEKCJI_GRACZA).to(urzadzenie=urzadzenie)
    generator_ramki = sv.get_video_frames_generator(source_path=sciezka_wideo)
    sledziciel = sv.ByteTrack(minimum_consecutive_frames=3)
    for ramka in generator_ramki:
        wynik = model_detekcji_gracza(ramka, imgsz=1280, verbose=False)[0]
        detekcje = sv.Detections.from_ultralytics(wynik)
        detekcje = sledziciel.update_with_detections(detekcje)
        ...
        ramka_oznaczona = OZNACZNIK_ETYKIEK_ELIPSY.annotate(
...
def uruchom_klasyfikacje_druzyn(sciezka_wideo: str, urzadzenie: str) -> Iterator[np.ndarray]:
    ...
    przycinki = []
    for ramka in tqdm(generator_ramki, desc='zbieranie przycinków'):
        wynik = model_detekcji_gracza(ramka, imgsz=1280, verbose=False)[0]
        detekcje = sv.Detections.from_ultralytics(wynik)
        przycinki += uzyskaj_przycinki(ramka, detekcje[detekcje.class_id == KLASA_GRACZA])

    klasyfikator_druzyn = KlasyfikatorDruzyn(urzadzenie=urzadzenie)
    klasyfikator_druzyn.dopasuj(przycinki)
    ...
    for ramka in generator_ramki:
        ...
        przycinki = uzyskaj_przycinki(ramka, gracze)
        id_druzyny_graczy = klasyfikator_druzyn.przewiduj(przycinki)
